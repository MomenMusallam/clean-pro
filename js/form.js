...
import { toggleLoadingState, resetFormFields, debounce } from './modules/utils.js';
...
class CleaningFormApp {
    ...
    async init() {
        ...
        try {
            ...
            this.showErrorMessage('Failed to initialize form. Please refresh the page.');
        }
    }
    ...
    async setupEventListeners() {
        ...
        if (termsCheckbox) {
            termsCheckbox.addEventListener('change', this.handleTermsChange.bind(this));
        }
    ...
    setupRadioButtons() {
        document.addEventListener('click', (e) => {
            ...
            const radioInput = e.target.type === 'radio' ? e.target : 
                              e.target.closest('.card-label')?.querySelector('input[type=\"radio\"]');
            
            if (radioInput && radioInput.type === 'radio') {
                // Remove selected class from all cards in the same group
                document.querySelectorAll(`input[name=\"${radioInput.name}\"]`).forEach(radio => {
        ...
        const renderSelectedDates = (dates) => {
            ...
            dates.forEach((d, index) => {
                ...
                div.addEventListener('click', () => {
                    const newDates = this.flatpickrInstance.selectedDates.filter((_, i) => i !== index);
    ...
    setActiveTab(tabName) {
        ...
        const activeSection = document.querySelector(`.tab-section[data-tab=\"${tabName}\"]`);
        if (activeSection) {
    ...
    setupAddressToggles() {
        const cleaningAddressCheckbox = document.getElementById('separateCleaningAddress');
        ...
        if (cleaningAddressCheckbox && cleaningAddressSection) {
            ...
            cleaningAddressCheckbox.addEventListener('change', (e) => {
                cleaningAddressSection.style.display = e.target.checked ? 'block' : 'none';
                if (!e.target.checked) {
        ...
        const contactPersonCheckbox = document.getElementById('separateContactPerson');
        ...
        if (contactPersonCheckbox && contactPersonSection) {
            ...
            contactPersonCheckbox.addEventListener('change', (e) => {
                contactPersonSection.style.display = e.target.checked ? 'block' : 'none';
                if (!e.target.checked) {
                    contactPersonSection.querySelectorAll('input, select, textarea').forEach(field => {
                        if (field.type === 'checkbox' || field.type === 'radio') {
                            field.checked = false;
                        } else {
                            field.value = field.tagName === 'SELECT' ? '0' : '';
    ...
    addOptionalService(serviceId) {
        ...
        const dropdownItem = document.querySelector(`.dropdown-item[data-value=\"${serviceId}\"]`);
    ...
    removeOptionalService(serviceId) {
        ...
        const availableBoxes = this.availableOptionalsByService[this.currentTab] || [];
        if (availableBoxes.includes(serviceId)) {
            const dropdownItem = document.querySelector(`.dropdown-item[data-value=\"${serviceId}\"]`);
            if (dropdownItem) dropdownItem.style.display = 'block';
        }

        this.optionalServices = this.optionalServices.filter(id => id !== serviceId);
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            if (response.success) {
                uiManager.showSuccess('Form submitted successfully! We will contact you soon.');
    ...
    validateForm() {
        ...
        if (!tabErrors.isValid) {
            isValid = false;
            allErrorFields.push(...tabErrors.fields.map(f => ({ field: f, order: 2 }))); // Main service second
        }
        ...
        if (!optionalErrors.isValid) {
            isValid = false;
            allErrorFields.push(...optionalErrors.fields.map(f => ({ field: f, order: 3 }))); // Optional services third
        }

        // Validate personal information
        const personalFields = [
            'billingEmail', 'billingMobile', 'billingFirstName', 
            'billingSecondName', 'billingStreet', 'billingNo', 
            'billingZip', 'billingCity', 'billingCountry', 'billingSalutation'
        ];
        ...
        switch (this.currentTab) {
            case 'normal-cleaning':
                ...
                if (!areaForNormal?.value || parseFloat(areaForNormal.value) <= 0) {
                    this.showFieldError(areaForNormal, 'Please enter area in square meters');
                    isValid = false;
                ...
                if (!this.validator.validateRadioGroup('contaminationForNormal')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForNormal"]');
            ...
            case 'windows-cleaning':
                ...
                if (!this.validator.validateRadioGroup('contaminationForWindowCleaning')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForWindowCleaning"]');
                    if (radio) errorFields.push(radio);
                }
                break;
            ...
            case 'carpet':
                ...
                if (!this.validator.validateRadioGroup('contaminationForCarpet')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForCarpet"]');
                    if (radio) errorFields.push(radio);
                }
                break;
            ...
            case 'upholstery-cleaning':
                ...
                if (!this.validator.validateRadioGroup('contaminationForUpholstery')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForUpholstery"]');
                    if (radio) errorFields.push(radio);
                }
                break;
            ...
            case 'spring-cleaning':
                ...
                if (!this.validator.validateRadioGroup('contaminationForSpringCleaning')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForSpringCleaning"]');
                    if (radio) errorFields.push(radio);
                }
                break;
            ...
            case 'cleaning':
                ...
                if (!this.validator.validateRadioGroup('contaminationForCleaning')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForCleaning"]');
                    if (radio) errorFields.push(radio);
                }
                break;
            ...
            case 'messie-apartment':
                ...
                if (!this.validator.validateRadioGroup('contaminationForMessieApatment')) {
                    isValid = false;
                    const radio = document.querySelector('input[name="contaminationForMessieApatment"]');
                    if (radio) errorFields.push(radio);
                }
                break;
        }
        ...
        this.optionalServices.forEach(boxId => {
            ...
            switch(boxId) {
                case 'box-1': // Window Cleaning Optional
                    const reasonWindow = document.getElementById('reasonForWindowCleaningOptional');
                    const casementWindow = document.getElementById('casementForWindowCleaningOptional');
                    const heightWindow = document.getElementById('heightInputForWindowCleaningOptional');
                    ...
                    if (!casementWindow?.value || parseFloat(casementWindow.value) <= 0) {
                        this.showFieldError(casementWindow, 'Please enter number of window sashes');
                        isValid = false;
                    ...
                    if (!this.validator.validateRadioGroup('contaminationForWindowCleaningOptional')) {
                        isValid = false;
                        const radio = document.querySelector('input[name="contaminationForWindowCleaningOptional"]');
                        if (radio) errorFields.push(radio);
                    }
                    break;

                case 'box-2': // Carpet Cleaning Optional
                    const areaCarpet = document.getElementById('totalAreaForCarpetOptional');
                    ...
                    if (!this.validator.validateRadioGroup('contaminationForCarpetOptional')) {
                        isValid = false;
                        const radio = document.querySelector('input[name="contaminationForCarpetOptional"]');
                        if (radio) errorFields.push(radio);
                    }
                    break;
                ...
                case 'box-3': // Upholstery Cleaning Optional
                    ...
                    // }
                    if (!this.validator.validateRadioGroup('contaminationForUpholsteryOptional')) {
                        isValid = false;
                        const radio = document.querySelector('input[name="contaminationForUpholsteryOptional"]');
                        if (radio) errorFields.push(radio);
                    }
                    break;
                ...
                case 'box-4': // Normal Cleaning Optional
                    ...
                    if (!this.validator.validateRadioGroup('contaminationForNormalOption')) {
                        isValid = false;
                        const radio = document.querySelector('input[name="contaminationForNormalOption"]');
                        if (radio) errorFields.push(radio);
                    }
                    break;
            }
    ...
    finishInitialization() {
        ...
        document.dispatchEvent(new CustomEvent('formAppInitialized', {
            detail: { app: this }
        }));
