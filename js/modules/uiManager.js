...
export class UIManager {
  ...
  setupCounters() {
    document.addEventListener("click", (e) => {
      if (
        ...
        const button = e.target.classList.contains("btn-plus")
          ? e.target
          : e.target.closest(".btn-plus");
        ...
        const button = e.target.classList.contains("btn-minus")
          ? e.target
          : e.target.closest(".btn-minus");
  ...
  incrementCounter(button) {
    ...
    const input = counterBox.querySelector(
  ...
  setupTooltips() {
    document
      .querySelectorAll(".cleaning-request label[data-full-text]")
      .forEach((label) => {
  ...
  showBox(boxId) {
    ...
    const dropdownItem = document.querySelector(
      `.dropdown-item[data-value="${boxId}"]`
    );
    if (dropdownItem) {
  ...
  setupAccordions() {
    ...
    if (firstAccordion && typeof bootstrap !== "undefined") {
      const collapse = new bootstrap.Collapse(firstAccordion, {
        toggle: false,
      });
      collapse.show();
    }
